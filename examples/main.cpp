...
int main() {
  ...
  printQDMI(
      "AWS_QDMI_device_session_query_device_property(session, QUBITSNUM, ...)",
  ...
  if (ret == QDMI_SUCCESS) {
    std::cout << "    -> Value: "
              << (status == QDMI_DEVICE_STATUS_IDLE   ? "IDLE"
                  : status == QDMI_DEVICE_STATUS_BUSY ? "BUSY"
                                                      : "OTHER")
              << "\n";
  }
  ...
  printQDMI(
      "AWS_QDMI_device_session_query_device_property(session, OPERATIONS, ...)",
  ...
  std::cout
      << "   [AWS] -> CreateQuantumTaskRequest::SetOutputS3Bucket(S3BUCKET)\n";
  std::cout
      << "   [AWS] -> CreateQuantumTaskRequest::SetOutputS3KeyPrefix(prefix)\n";
  std::cout
  ...
  if (ret != QDMI_SUCCESS) {
    ...
    if (ret == QDMI_SUCCESS) {
      const char* statusStr = jobStatus == QDMI_JOB_STATUS_CREATED   ? "CREATED"
                              ...
                              : jobStatus == QDMI_JOB_STATUS_CANCELED
                                  ? "CANCELED"
                                  : "UNKNOWN";
      ...
      if (ret == QDMI_SUCCESS && jobStatus == QDMI_JOB_STATUS_DONE) {
        ...
        if (ret == QDMI_SUCCESS && shotsSize > 0) {
          ...
          ret = AWS_QDMI_device_job_get_results(
              job, QDMI_JOB_RESULT_SHOTS, shotsSize, shotsData.data(), nullptr);
        ...
        ret = AWS_QDMI_device_job_get_results(job, QDMI_JOB_RESULT_HIST_KEYS, 0,
                                              nullptr, &keysSize);
        ...
        if (ret == QDMI_SUCCESS && valuesSize > 0) {
          ...
          if (keys.size() == 2) {
            bool hasBellState = (keys[0] == "00" || keys[0] == "11") &&
                                (keys[1] == "00" || keys[1] == "11");
            std::cout << "\n";
  ...
  std::cout << "  AWS_QDMI_device_session_init()            -> BraketClient + "
               "GetDevice\n";
  ...
  std::cout << "  AWS_QDMI_device_session_create_device_job() -> (internal "
               "allocation)\n";
  std::cout
      << "  AWS_QDMI_device_job_set_parameter()       -> (store job config)\n";
  std::cout << "  AWS_QDMI_device_job_query_property()      -> (return stored "
               "values)\n";
  std::cout
  ...
  std::cout << "  AWS_QDMI_device_job_wait()                -> (poll "
               "GetQuantumTask)\n";
  std::cout
      << "  AWS_QDMI_device_job_get_results()         -> S3Client::GetObject\n";
  std::cout
      << "  AWS_QDMI_device_job_cancel()              -> CancelQuantumTask\n";
  std::cout
      << "  AWS_QDMI_device_job_free()                -> (internal cleanup)\n";
